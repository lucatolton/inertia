	...
	http(req, res, next = () => res.end("")) {
		...
		if (proxyOptions.headers["cookie"]) {
			...
			cookie_array.forEach((cookie) => {
				...
				if (
					proxyURL.hostname.includes(
						cookie_name.split("@").splice(1).join()
					)
				)
					new_cookie.push(
						cookie_name.split("@").splice(0, 1).join() +
							"=" +
							cookie_value
					);
			});
		...
		if (
			...
			proxyOptions.localAddress = this.config.localAddress[
				...
				proxyResponse
					...
					.on("end", () => {
						...
						proxify.url = (url) => {
							if (
								url.match(
									/^(#|about:|data:|blob:|mailto:|javascript:|{|\*)/
								)
							)
								return url;
							...
							if (
								...
								return (
									this.prefix +
									this.proxifyRequestURL(url.href)
								);
						...
						proxify.html = (body) => {
							...
							document
								...
								.forEach((node) => {
									if (node.src)
										node.src = proxify.attribute(node.src);
									if (
										node.tagName.toLowerCase() ==
											"script" &&
										node.innerHTML != ""
									)
										node.innerHTML = proxify.js(
											node.innerHTML
										);
								});
							...
							document
								.querySelectorAll("a, link, area")
								.forEach((node) => {
									if (node.href)
										node.href = proxify.attribute(
											node.href
										);
								});
							...
							document
								...
								.forEach((node) => {
									node.textContent = proxify.css(
										node.textContent
									);
								});

							// Creating injection script element.
							const inject_script = document.createElement(
								"script"
							);
						...
						if (rawData.length != 0)
							switch (proxyResponse.headers["content-encoding"]) {
								case "gzip":
									sendData = zlib.gunzipSync(
										Buffer.concat(rawData)
									);
									break;
								case "deflate":
									sendData = zlib.inflateSync(
										Buffer.concat(rawData)
									);
									break;
								case "br":
									sendData = zlib.brotliDecompressSync(
										Buffer.concat(rawData)
									);
									break;
								default:
		...
		if (!res.writableEnded)
			req.on("data", (data) => makeRequest.write(data)).on("end", () =>
				makeRequest.end()
			);
	}
	// Websocket Proxy
	ws(server) {
		new WebSocket.Server({ server: server }).on(
			"connection",
			(cli, req) => {
				...
				Object.entries(req.headers).forEach(
					([header_name, header_value]) => {
						if (header_name == "sec-websocket-protocol")
							header_value
								.split(", ")
								.forEach((proto) => protocol.push(proto));
						if (
							header_name.startsWith("cf-") ||
				...
				if (
					...
					options.localAddress = this.config.localAddress[
						Math.floor(
							Math.random() * this.config.localAddress.length
						)
					];
