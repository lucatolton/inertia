...
try {
	...
	window.alloyLocation = new Proxy(
		{},
		{
			set(obj, prop, value) {
				...
				console.log(
					proxify.url(alloy.url.href.replace(alloy.url[prop], value))
				);
			...
			switch (type) {
				...
				default:
					...
					if (url.startsWith("https://") || url.startsWith("http://"))
						...
						url = new URL(
							alloy.url.href.split("/").slice(0, -1).join("/") +
								"/" +
								url
						);
		...
		element_array.forEach((element) => {
			Object.defineProperty(element.prototype, "innerHTML", {
				set(value) {
					...
					elem.querySelectorAll("object[data]").forEach((node) =>
						node.setAttribute("data", node.getAttribute("data"))
					);
					elem.querySelectorAll("a[href], link[href], area[href").forEach(
						(node) =>
	...
	document.write = new Proxy(document.write, {
		apply(target, thisArg, args) {
			var processedHTML = new DOMParser().parseFromString(
				args[0],
				"text/html"
			);
